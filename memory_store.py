...
def _parse_datetime(dt_val) -> datetime.datetime | None:
    if dt_val is None:
        return None
...
def check_escalation(db: firestore.Client, zone: str | None) -> dict | None:
    """
    ...
    # Requête avec filtre d'égalité sur la zone (index simple Firestore automatique).
    # Le filtrage temporel se fait en mémoire pour éviter d'exiger la création
    # manuelle d'un index composite (zone ASC, received_at ASC).
    query = (
    ...
    for doc in query.stream():
        data = doc.to_dict()
        doc_received = _parse_datetime(data.get("received_at"))
        if doc_received and doc_received >= window_start:
