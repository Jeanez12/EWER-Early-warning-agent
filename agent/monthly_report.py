    ...
    simplified = [
        {
            ...
            "indicators_detected": [
                i.get("category") for i in r.get("indicators_detected", [])
            ],
        }
        for r in reports
    ]
...
def _upload_map_to_storage(html_content: str, year: int, month: int) -> str:
    ...
    logger.info("Carte mensuelle uploadée: %s", blob.public_url)
...
def generate_monthly_report(year: int | None = None, month: int | None = None) -> dict:
    ...
    reports = _fetch_month_reports(db, year, month)

    if not reports:
        logger.info("Aucun rapport trouvé pour %04d-%02d, rapport vide généré.", year, month)
        return {
            "year": year, "month": month, "total_reports": 0,
    ...
    map_url = _upload_map_to_storage(map_html, year, month)
    ...
    db.collection("ewer_monthly_reports").document(f"{year}-{month:02d}").set({
        **result,
        "generated_at": now,
    })

    logger.info("Rapport mensuel %04d-%02d généré: %d rapports, carte: %s",
                year, month, result.get("total_reports", len(reports)), map_url)
    return result
